...
3. 点扩展图标。**面板地址已经按 `http://127.0.0.1:8000` 填好**（`uvicorn` 和
   `docker compose` 两条路都在这儿），只需要填 **API token**（就是 `SFD_API_TOKEN`），
   点「保存」。面板开在别处就改掉那一格——但**别写 `localhost`**，理由见下面第一个坑。
...
**三、第一次授权那一下，弹窗会自己关掉，而且看起来什么都没发生。**
Chrome 在显示「是否允许访问 …」这个授权框时**会把扩展的小窗口关掉**。窗口一销毁，点击处理
函数里 `await` 之后的代码就再也不会执行——不发消息、不导入、也不报错。所以第一次授权的那一次
点击**必然丢掉自己的结果**。这不是出错：**重新点一下扩展图标，再点一次「导入」**即可，那时
权限已经在了、不再弹框、窗口也就不会关。

现在每次导入的结果都会先写进 `chrome.storage.local` 再回复，所以就算窗口已经没了，
下次打开弹窗也会显示「上次导入…」。弹窗在权限还没授予时也会提前把这件事说清楚。
