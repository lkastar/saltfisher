...
import { findSellerRule, sellerRuleName } from "./monitorRules";
...
describe("findSellerRule", () => {
  it("finds the rule watching that seller", () => {
    const existing = rule({ id: 7, name: "卖家：小顾数码", seller_id: ID, seller_nick: "小顾数码" });
  ...
  it("never matches a keyword rule", () => {
    ...
    expect(findSellerRule([...keywordRules, rule({ id: 3, seller_id: ID })], ID)?.id).toBe(3);
  });
});
